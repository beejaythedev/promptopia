"use client"
 
import Link from 'next/link'
import Image from 'next/image'
import { useState, useEffect } from 'react'
import { signIn, signOut, useSession, getProviders } from 'next-auth/react'


const Nav = () => {
    const { data: session } = useSession(); // Replace with actual logic to check if user is logged in

    const [providers, setProviders] = useState(null);
    const [toggleDropdown, setToggleDropdown] = useState(false);

    useEffect(() =>{
            const setUpProviders = async () => {
                const response = await getProviders();
                setProviders(response);
            }
            setUpProviders();
    }, [])
  return (
    <nav className='flex justify-between items-center w-full mb-16 p-5'>
        <Link href='/' className='flex gap-2 flex-center'>
        <Image src="/assets/images/logo.svg" alt="Promptopia Logo" width={30} height={30} className='object-contain' />
        <p className='logo_text text-xl'>Promptopia</p>
        </Link>
       

       {/* Desktop Navigation */}
       <div className='sm:flex hidden'>
            {session?.user ? (
                <div className='flex gap-3 md:gap-5'>
                    <Link href='/create-prompt' className='black_btn border border-black py-2 px-5 rounded-full text-white bg-black'>Create Post</Link>

                    <button type='button' onClick={signOut} className='outline_btn border py-e px-5 rounded-full'>Sign Out</button>

                    <Link href='/profile'>
                        <Image src={session?.user.image} width={37}
                        height={37}
                        className='rounded-full'
                        alt='profile' />
                    </Link>
                </div>
            ): (
                <>
                    {providers && Object.values(providers).map((provider) => (
                        <button type='button' key={provider.name} onClick={() => signIn(provider.id)} className='black_btn border border-black py-2 px-5 rounded-full text-white bg-black'>Sign In</button>
                    ))}
                </>
            )}

       </div>

       {/* Mobile Navigation */}
       <div className='sm:hidden flex items-center relative'>
            {session?.user ? (
                <div className=''>
                    <Image src="/assets/images/logo.svg"
                        width={37}
                        height={37}
                        className='rounded-full'
                        alt='profile'
                        onClick={() => setToggleDropdown((prev) => !prev)} />

                        {toggleDropdown && (
                            <div className='dropdown flex flex-col absolute top-12 right-0 bg-white border border-gray-300 rounded-lg shadow-lg p-5 text-center w-50 z-10'>
                                <Link href='/profile' className='dropdown_link' onClick={() => setToggleDropdown(false)}>My Profile</Link>

                                <Link href='/create-prompt' className='dropdown_link' onClick={() => setToggleDropdown(false)}>Create Prompt</Link>

                                <button type='button' onClick={() => {
                                    setToggleDropdown(false);
                                    signOut();
                                }} className='mt-5 w-full black_btn border border-black py-2 px-5 rounded-full text-white bg-black'>Sign Out</button>        
                            </div>
                        )}
                </div>
            ): (
                <>
                    {providers && Object.values(providers).map((provider) => (
                        <button type='button' key={provider.name} onClick={() => signIn(provider.id)} className='black_btn border border-black py-2 px-5 rounded-full text-white bg-black'>Sign In</button>
                    ))}
                </>
                )}
       </div>
    </nav>
  )
}

export default Nav